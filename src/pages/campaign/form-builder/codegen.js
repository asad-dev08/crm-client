...
export const getGeneratedCode = (element) => {
  switch (element.type) {
    case "div":
      return (
        <div
    ...
    case "radio":
      return (
        <div className="relative">
          {element.radioLabel && <Typography>{element.radioLabel}</Typography>}
          <Form.Item
            id={element.id}
    ...
    case "list":
      return (
        <div className="bg-slate-50 w-full relative">
          ...
          <List
            dataSource={element.listOptions.length > 0 && element.listOptions}
            style={{
              ...element.customStyle,
            }}
            renderItem={(item) => (
