...
import {
  deleteEmailTemplate,
  getEmailTemplate,
  getEmailTemplatesWithPagination,
...
import EmailTemplateListHeader from "./EmialTemplateListHeader";
...
const EmailTemplateList = () => {
  ...
  const handleMenuClick = async (e, row) => {
    if (e.key === "edit") {
      const response = await dispatch(getEmailTemplate(row.id));
      const data =
  ...
  const handleDeleteModalOk = async () => {
    ...
    const response = await dispatch(deleteEmailTemplate(selectedRow.id));
    if (response) {
      toast.success("EmailTemplate deleted", { duration: 3000 });
  ...
  const columns = [
    { columnName: "template_name", columnShow: "template name" },
    { columnName: "event_name", columnShow: "Event Name" },
    { columnName: "email_subject", columnShow: "Email Subject" },
