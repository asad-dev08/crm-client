...
const CreateExpenseCategory = ({ onClose, open, data, isAdd }) => {
  ...
  const onFinish = async (values) => {
    ...
    try {
      if (isAdd) {
        await dispatch(saveExpenseCategory(model))
          .then((response) => {
            if (
              response &&
              response.payload &&
        ...
        await dispatch(updateExpenseCategory(model))
          .then((response) => {
            if (
              response &&
              response.payload &&
      ...
      >
        <Form.Item
          ...
          rules={[
            {
              required: true,
              message: "Enter ExpenseCategory Name",
            },
          ]}
        >
          <Input />
