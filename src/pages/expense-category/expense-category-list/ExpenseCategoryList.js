...
import {
  ...
  getExpenseCategory,
  getExpenseCategorysWithPagination,
} from "../../../redux/expense-category/expenseCategorySlice";
...
import ExpenseCategoryListHeader from "./ExpenseCategoryListHeader";
import CreateExpenseCategory from "../create-expense-category/CreateExpenseCategory";
...
const ExpenseCategoryList = () => {
  ...
  const handleMenuClick = async (e, row) => {
    if (e.key === "edit") {
      const response = await dispatch(getExpenseCategory(row.id));
      const data =
  ...
  const handleDeleteModalOk = async () => {
    ...
    const response = await dispatch(deleteExpenseCategory(selectedRow.id));
    if (response) {
      toast.success("ExpenseCategory deleted", { duration: 3000 });
