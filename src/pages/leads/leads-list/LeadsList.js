...
const LeadsList = () => {
  ...
  const [deleteLeadL, setDeleteLeadL] = useState(false);
  ...
  const handleMenuClick = async (e, row) => {
    if (e.key === "edit") {
      const response = await dispatch(getLead(row.id));
      const data =
  ...
  const handleDeleteModalOk = async () => {
    ...
    const response = await dispatch(deleteLead(selectedRow.id));
    if (response) {
      toast.success("Lead deleted", { duration: 3000 });
      setDeleteLeadL(true); // Trigger data refetch
  ...
  const columns = [
    ...
    { columnName: "source_name", columnShow: "Source" },
    { columnName: "status_name", columnShow: "status" },
    { columnName: "assigned_user", columnShow: "Assigned User" },
  ...
  useEffect(() => {
    if (deleteLeadL) {
      ...
      setDeleteLeadL(false); // Reset delete status
    }
  }, [deleteLeadL]);
  return (
    <div>
