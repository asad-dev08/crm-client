...
const StatusList = () => {
  ...
  const handleMenuClick = async (e, row) => {
    if (e.key === "edit") {
      const response = await dispatch(getLeadStatus(row.id));
      const data =
  ...
  const handleDeleteModalOk = async () => {
    ...
    const response = await dispatch(deleteLeadStatus(selectedRow.id));
    if (response) {
      toast.success("LeadStatus deleted", { duration: 3000 });
