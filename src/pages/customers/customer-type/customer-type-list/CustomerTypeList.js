...
import {
  deleteCustomerType,
  getCustomerType,
  getCustomerTypesWithPagination,
} from "../../../../redux/customer-type/customerTypeSlice";
...
import CreateCustomerType from "../create-customer-type/CreateCustomerType";
import CustomerTypeListHeader from "./CustomerTypeListHeader";
...
const CustomerTypeList = () => {
  ...
  const handleMenuClick = async (e, row) => {
    if (e.key === "edit") {
      const response = await dispatch(getCustomerType(row.id));
      const data =
  ...
  const handleDeleteModalOk = async () => {
    ...
    const response = await dispatch(deleteCustomerType(selectedRow.id));
    if (response) {
      toast.success("CustomerType deleted", { duration: 3000 });
