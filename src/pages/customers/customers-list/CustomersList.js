...
const CustomersList = () => {
  ...
  const handleMenuClick = async (e, row) => {
    if (e.key === "edit") {
      const response = await dispatch(getCustomer(row.id));
      const data =
  ...
  const handleDeleteModalOk = async () => {
    ...
    const response = await dispatch(deleteCustomer(selectedRow.id));
    if (response) {
      toast.success("Customer deleted", { duration: 3000 });
  ...
  const columns = [
    { columnName: "company_name", columnShow: "Company Name" },
    { columnName: "vat_number", columnShow: "VAT number" },
